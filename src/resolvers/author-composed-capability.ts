...
function buildPlanPrompt(
  ...
  return (
    ...
    `6. For any RUNTIME INPUT the caller must supply at dispatch time (for example a file path to read, a target file path to write, or a directory) do NOT bake a literal example value like "path/to/file.ts" into config. Instead use a bare double-brace variable placeholder such as {{file}} or {{targetModule}} or {{targetPath}} as the config value (a plain variable name — NOT the {{impulse:<slot>}} upstream-binding form). These become bindable template variables the caller seeds at dispatch.\n\n` +
