 ...
 */
/**
 * MAX_GOAL_LEN guard rationale: prevents goal text from exceeding a maximum
 * length to ensure downstream processing remains tractable, avoid token limit
 * violations in LLM context windows, and maintain reasonable goal clarity
 * boundaries. Critical for goal-host /recommend scoring reliability and
 * downstream resolver dispatch safety.
 */
...
// MAX_GOAL_LEN rationale: 8192 chars is chosen to fit comfortably within the
// goal-host-vessel token budget for a single dispatched goal, matches the
// substrate catalogue's per-goal payload fitting constraints, and keeps the
 ...
 * MAX_GOAL_LEN — guard rationale.
 *
 * Upper bound (in characters) on the goal payload accepted by this resolver
 * before dispatch to goal-host-vessel. This constant exists to:
 *  - Prevent goal text from exceeding the resolver's processing capacity.
 *  - Ensure compatibility with downstream prompt assembly (goal-host-vessel
 *    embeds the goal into an LLM prompt with fixed context budget).
 *  - Avoid token overflow in downstream LLM providers by keeping the goal
 *    well within a single-request context window.
 *  - Provide a hard backstop against runaway or adversarial payloads on the
 *    dispatch surface.
 *
 ...
 */
// MAX_GOAL_LEN guard rationale:
//   Enforces a maximum goal text length to prevent token overflow in LLM
//   prompts and ensure goal-host /recommend can score goals within reasonable
