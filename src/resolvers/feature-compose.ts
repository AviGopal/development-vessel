...
Emit ONE JSON object, no markdown fences, with this exact schema:
{
  ...
  "ops": [
    ...
    // edit an EXISTING file by CONTENT ANCHOR. Use only when the file is NOT shown with line numbers, or the anchor is one short unique line you can copy exactly. old_string MUST be a verbatim unique substring of the CURRENT file — byte for byte, never paraphrased, reformatted or renamed. A re-derived anchor is REFUSED and the whole compose fails:
    { "kind": "edit", "path": "repos/<vessel>/<subpath>", "old_string": "<verbatim current text>", "new_string": "<replacement>", "rationale": "<why>" },
    // replace a LINE RANGE — PREFER THIS whenever the target file is shown with line numbers below. It asks for TWO short boundary lines instead of a whole verbatim block, and apply DRIFT-REFUSES if either is wrong, so a slip is caught rather than mis-applied. Measured 2026-08-31: content anchors are refused ~11 times per 12h because the model PARAPHRASES instead of transcribing — it emitted if (slot.mtime < now - SLOT_STALE_MS) { where the file reads if (now - st.mtimeMs > SLOT_STALE_MS) { (renamed the field AND inverted the comparison), and kill -9 "$__cpid" 2>/dev/null where the file reads kill -9 -$__cpid 2>/dev/null (dropped the leading minus, turning a process-GROUP kill into a process kill). Every such refusal sets apply_failed, which SKIPS the repair loop entirely, so the compose dies with no second chance. Also REQUIRED when a change spans N identical, ADJACENT blocks that cannot be uniquely content-anchored (their surrounding context is also identical). Target files in GROUND TRUTH are shown as: line-number, then a TAB, then the line text. start_line/end_line are those 1-indexed numbers (inclusive); expect_first_line/expect_last_line are the VERBATIM <line text> of those two boundary lines — the text AFTER the tab only, NOT the number or tab (apply DRIFT-REFUSES on any mismatch); new_string is the full replacement for the whole range (use "" to delete it):
