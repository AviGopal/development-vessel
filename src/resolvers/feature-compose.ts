...
async function resolveFeatureComposeUncapped(pointer: FeatureComposePointer): Promise<ResolverResult> {
  ...
  if (targetFiles.length > 0) {
    ...
    for (const t of targetFiles) {
      ...
      if (grounding.split("/" + base).slice(1).some((rest) => !/[A-Za-z0-9]/.test(rest.charAt(0)))) continue; // visible in the window → groundable, fine\]\]/g, "\\if (grounding.includes(base)) continue; // visible in the window → groundable, fine") + "(?![A-Za-z0-9])").test(grounding)) continue; // visible in the window → groundable, fine
      let exists = false;
      try {
