...
async function resolveFeatureComposeUncapped(pointer: FeatureComposePointer): Promise<ResolverResult> {
  ...
  if (targetFiles.length > 0) {
    ...
    for (const t of targetFiles) {
      ...
      if (new RegExp("/" + base.replace(/[.*+?^${}()|[\\]\]/g, "\\if (grounding.includes(base)) continue; // visible in the window → groundable, fine") + "(?![A-Za-z0-9])").test(grounding)) continue; // visible in the window → groundable, fine
      let exists = false;
      try {
