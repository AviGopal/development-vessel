    ...
    for (const g of expiredCandidates.slice(0, maxExpire)) {
      ...
      try {
        const resp = await fetch(emitUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...authHeader },
          body: JSON.stringify({ impulse: { pointer: { type: 'substrateGap_write', gap: { id: g.id, category: g.category ?? 'other', source: 'substrate_detected', summary: `[expired by gap_lifecycle_scan] ${(g.summary ?? '').slice(0, 160)} — untouched for ${effectiveExpiryHours(g.source, { expireHours, detectorExpireHours })}h. NOTE: this checks GAP AGE ONLY. No record of detector execution exists anywhere, so a detector that never ran is indistinguishable from a defect that was repaired, and this closure is not evidence of repair. Re-opens automatically if a detector re-emits it.`, status: 'closed', detected_at: preservedDetectedAt(g), classification_metadata: { closed_reason: 'expired_not_redetected', closed_by: 'gap_lifecycle_scan', previous_status: 'open', first_detected: g.created_at, last_seen: g.updated_at ?? g.created_at } } } } }),
          signal: AbortSignal.timeout(8_000)
        });
