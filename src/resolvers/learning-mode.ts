...
export async function resolveLearningMode(pointer: LearningModePointer): Promise<ResolverResult> {
  const p = pointer;
  const devBase = (p.devVesselUrl ?? process.env["DEV_VESSEL_ENDPOINT"] ?? "http://127.0.0.1:8090").replace(/\/$/, "");
  const DEV = devBase.endsWith("/v2/impulses/resolve") ? devBase : `${devBase}/v2/impulses/resolve`;
  const ACT = p.activityApiUrl ?? process.env["ACTIVITY_API_ENDPOINT"] ?? "http://127.0.0.1:8080";
