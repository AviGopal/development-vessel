...
async function triggerMitosisTick(a: {
  ...
  try {
    ...
    const cut = await resolveVesselMitosisCutover({
      ...
      evaluation_evidence: { verdict: "FAVORABLE", base_success_rate: 1, mitosis_success_rate: 1, cited_trace_ids: [], cited_check_names: citedChecks.length ? citedChecks : ["static_evaluate"], gap_id: a.gapId, proposal_id: a.proposalId, refused_surql_cutover: true },
      gap_id: a.gapId,
