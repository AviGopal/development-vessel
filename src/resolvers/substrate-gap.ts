  ...
  if (!skipComposeTrigger && (action === "created" || (action === "updated" && (summaryChanged || reopened))) && (gap.status ?? "open") === "open") {
    ...
    if (!g.__gapComposeLastTrigger || nowMs - g.__gapComposeLastTrigger > 60_000) {
      ...
      if (gd.__composeDrainInflight === true) {
        ...
        const proc = Bun.spawn(["systemctl","start","gap-compose.service"], { stdout: "inherit", stderr: "inherit", onExit: (proc, exitCode, signalCode) => { if (exitCode !== 0) { console.error(`[substrate-gap] gap-compose.service failed with exit code ${exitCode} and signal ${signalCode}`); } } });
        console.log(`[substrate-gap] event-driven gap-compose pickup triggered by ${incoming.id}: systemctl started gap-compose.service`);
