    ...
    void fetch(`${ep}/v2/activities/executions`, {
      ...
      body: JSON.stringify({
        ...
        metadata: {
          ...
          newly_failing: Array.isArray((extra ?? {})["newly_failing"]) ? ((extra ?? {})["newly_failing"] as string[]).filter(f => !/ \[\d{4}m?s\]$/.test(f)) : [],
          timeout_failures: Array.isArray((extra ?? {})["newly_failing"]) ? ((extra ?? {})["newly_failing"] as string[]).filter(f => / \[\d{4}m?s\]$/.test(f)) : [],
        },
      }),
