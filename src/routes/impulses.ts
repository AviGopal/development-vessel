...
import { resolvePushPolicy, resolvePushPolicyWrite } from "../resolvers/push-policy.js";
...
import { resolveVesselExerciseScan } from "../resolvers/vessel-exercise-scan.js";
...
async function dispatchInner(pointer: AnyPointer): Promise<ResolverResult> {
  ...
  switch (pointer.type) {
    ...
    case "pushPolicy":
      return resolvePushPolicy(pointer as unknown as Parameters<typeof resolvePushPolicy>[0]);
    case "pushPolicy_write":
      return await resolvePushPolicyWrite(pointer as unknown as Parameters<typeof resolvePushPolicyWrite>[0]);
    ...
    case "attemptLedger":
      return resolveAttemptLedger(
        p as Parameters<typeof resolveAttemptLedger>[0],
      );
    case "unaccounted_landing_scan":
      return resolveUnaccountedLandingScan(
        p as Parameters<typeof resolveUnaccountedLandingScan>[0],
      );
    ...
    case "fileList": {
      const { resolveFileList } = await import("../resolvers/file-list.js");
      return resolveFileList(pointer as Parameters<typeof resolveFileList>[0]);
    }
    default:
